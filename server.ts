...
async function startServer() {
  ...
  // Rota informativa sobre administradores (gerenciados exclusivamente via Supabase)
  app.post('/api/admin/register', (_req, res) => {
    return res.status(403).json({
      success: false,
      error: 'Cadastro de administradores desabilitado pelo site. O gerenciamento de administradores deve ser feito exclusivamente no Supabase.',
    });
  });
